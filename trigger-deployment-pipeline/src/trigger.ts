...
export function parseInputs(
  ...
  // Trimmed, because a value that is only whitespace names no pipeline at all:
  // it would otherwise pass here, trigger nothing, and report success.
  if (raw.bucket.trim() === "") return err("Parameter 'aws-s3-bucket-name' is empty")
  if (raw.pipelines.trim() === "") return err("Parameter 'pipelines' is empty")
  if (raw.triggerType.trim() === "") return err("Parameter 'trigger-type' is empty")
  ...
  if (triggerType === "artifact" && raw.artifactParameters.trim() === "") {
    return err(
