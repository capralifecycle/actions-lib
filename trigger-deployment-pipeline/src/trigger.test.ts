...
describe("inputs are rejected", () => {
  ...
  test("when a required parameter is only whitespace, which names nothing", () => {
    // It would otherwise pass, trigger no pipeline at all, and report success.
    expect(error(parseInputs(raw({ pipelines: "   " }), exists([])))).toBe(
      "Parameter 'pipelines' is empty",
    )
    expect(error(parseInputs(raw({ bucket: " " }), exists([])))).toBe(
    ...
    expect(
      error(parseInputs(raw({ artifactParameters: "  " }), exists([]))),
    ).toContain("'artifact-parameters' must be set")
  })
